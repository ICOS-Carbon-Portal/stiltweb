...
import spray.json.*
import se.lu.nateko.cp.data.formats.netcdf.{NetCdfJson, Raster, RasterMessage, Stats}

object PekkoRasterMarshalling:
	...
	private def getJson(raster: Raster) =
		...
		HttpResponse(entity = HttpEntity(MediaTypes.`application/json`, rasterMessage.toJson.prettyPrint))

	private def getBinary(raster: Raster): HttpResponse =
		val nLon = raster.sizeLon
		val nLat = raster.sizeLat

		def makeBuffer(size: Int) = ByteBuffer.allocate(size * 8).order(ByteOrder.BIG_ENDIAN)
